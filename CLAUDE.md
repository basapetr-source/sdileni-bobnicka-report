...
- **The monthly job runs locally, not in GitHub Actions** (since 2026-09-12). EDC silently drops TCP :443 from GitHub's hosted runner IP ranges on all three `*.edc-cr.cz` hosts — DNS resolves, the handshake never completes. Verified from a runner that day: 20/20 connect attempts timed out over 10 minutes, so retrying cannot help. The job needs a Czech IP.
  ...
  - The task stores the account password so it runs without login. Registered by `register-task.ps1`, which builds the task from XML — `New-ScheduledTaskTrigger` cannot express a day of month at all, and `schtasks /sc MONTHLY /d` takes only one day (`/d 12,13` fails with `Invalid value for /D option`).
  - **The local clone uses `gh` as its git credential helper** (`git config --local credential.helper '!gh auth git-credential'`). Git Credential Manager hangs in session 0 — the non-interactive context a scheduled task runs in — because it tries to raise a UI prompt, and `git push` never returns. This is in `.git/config`, so it is **not** carried by a fresh clone: re-apply it after re-cloning, or the monthly push will hang. Verified working via `git push --dry-run` from the task.
  - `run-monthly-local.ps1` calls every external program through its `Invoke-Native` helper. Under `$ErrorActionPreference = 'Stop'`, PowerShell 5.1 turns each stderr line from a native program into a thrown `NativeCommandError` — and git writes ordinary messages like `Everything up-to-date` and `To https://…` to stderr, so a plain `& git … 2>&1` aborts the script on success. The helper drops to `Continue` around the call and judges the result by exit code.
  - `-Force` bypasses the guard, for catching up a month that is already partly in the repo.
